...
### Version of 15 October 2024
### Corrected 9 July 2025
...
code.path = paste0(main.path, 'Codes3/')
sim.path = paste0(main.path, 'Simulation Results/Results14OCT24/')
setwd(code.path)
...
nresam = 3 #250         #number of replications for bootstrap
...
plot.to.file = FALSE    #flag for printing plots in a file
means.only = TRUE       #flag for plotting results for means only
save.ws = FALSE         #flag for saving workspace

#DATA GENERATING SETTINGS
scenario = c(2,1,1,1)           #simulation scenario
popgen = FALSE                  #flag for generating true population data            
npop = 1e6                      #initial size of population for generating population data
...
#CORRELATION PARAMETERS FOR DEPENDENT RECRUITMENT RUNS
icorr = TRUE                   #flag for nonzero correlations
cortst = FALSE                 #flag for checking correlation
corval1 = 0.2                  #target correlation for dependence between T1 and R
corval2 = 0.2                  #target correlation for dependence between T2 and R
theta1 = 0.234                 #coefficient for dependence between T1 and R
theta2 = 0.234                 #coefficient for dependence between T2 and R
if (cortst) n=1e6
...
label = paste0(label0, ', n=', n, ', cor1=', corval1, ', cor2=', corval2)
t1t2z.file.name = paste0('t1t2z-',label0a,'.csv')       #filename for population data
...
### MAIN ROUTINES FOR SIMULATING DATA ###########################################
...
#FUNCTION TO GENERATE A DATAFRAME WITH VARIABLES T1, T2, AND Z
sim_T1T2Z <- function(nn,T1_setting,T2_setting) {
  ...
  #for those with the disease, a new death time is sampled 
  #  based on a death distribution after diagnosis
  ...
  data1 = filter(data, T2 > L_R)

  return(data1)
  
}
#END OF FUNCTION TO GENERATE A DATAFRAME WITH VARIABLES T1, T2, AND Z

#FUNCTION TO SET UP POPULATION DATA
psetup = function(nn,T1_setting,T2_setting) {
  
  #GENERATE (T1,T2,Z) DATA FOR A LARGE POPULATION
  #the subjects with T2 <= L_R are filtered out already at this stage
  popdata <- sim_T1T2Z(nn=npop, T1_setting, T2_setting)
  
  #CREATE T1, T2, Z FILE
  #this file is needed for the runs with correlations between (T1,T2) and R
  T1 = popdata$T1
  T2 = popdata$T2
  Z = popdata$Z
  t1t2z = cbind(T1, T2, Z)
  write.csv(t1t2z,t1t2z.file.name)
  
  #TRUE CIF COMPUTATION - BEFORE ROUNDING
  #THE CIF IS CONDITIONAL ON T2>L_R
  ciftru = rep(0,100)
  for (ig in 1:100) {
    ciftru[ig] = mean((popdata$T1 <= ig) & (popdata$T1 <= popdata$T2))
  }
    
  #TRUE CIF COMPUTATION FOR AJ ESTIMATOR
  #THE CIF IS CONDITIONAL ON T1>L_R AND T2>L_R
  ciftru.aj = ciftru
  if (ipre) {
    aj.select = (popdata$T1 > L_R) 
    popdata.a = filter(popdata, aj.select)
    for (ig in 1:100) {
      ciftru.aj[ig] = mean((popdata.a$T1 <= ig) & (popdata.a$T1 <= popdata.a$T2))
      }
    rm(popdata.a)
  }
   
  #CREATE FILE WITH TRUE POPULATION CIF CURVES
  ciftru.fin = cbind(ciftru,ciftru.aj)
  write.csv(ciftru.fin,ciftru.file.name)
  
  rm(popdata)
  print(noquote('Population data has been generated'))
  stopQuietly()
    
}
#END OF CODE FOR GENERATING POPULATION DATA

#CDF OF T1
t1.cdf = function(tvals) {
  if (T1_setting == 1) ans = ptrunc(tvals, spec="weibull", shape = 4, scale=140, a = 40)
  if (T1_setting == 2) ans = ptrunc(tvals, spec="weibull", shape = 4, scale=116, a = 40)
  if (T1_setting == 3) ans = pweibull(tvals$T1-20, shape=0.85, scale=170)
  return(ans)
}  
 
#FUNCTION TO GENERATE CDF OF QSCORE
q.cdf.gen = function(popdat, t2.cdf) {
  T1 = popdat$T1
  T2 = popdat$T2
  Z = popdat$Z
  Z1 = qnorm(t1.cdf(T1))
  Z2 = qnorm(t2.cdf(T2))
  qscore = theta1*Z1 + theta2*Z2 + Z
  q.cdf = ecdf(qscore)
  return(q.cdf)
}

#FUNCTION TO GENERATE BASIC DATAFRAME WITH VARIABLES T1, T2, R, AND C  
sim_data <- function(nn,T1_setting,T2_setting,R_setting,C_setting,
  L_R=40,U_R=69,t2.cdf,q.cdf,pr.enter.flg=FALSE) {
  
  #GENERATE (T1,T2,Z) DATA 
  data = sim_T1T2Z(nn,T1_setting,T2_setting)
  nnew = nrow(data)
  T1 = data$T1
  T2 = data$T2
  Z = data$Z
  
  #GENERATE U(0,1) VARIABLE FOR R
  qscore = NULL
  if (icorr) {
    trm1 = pmin(t1.cdf(T1),1-1e-8)
    trm1 = pmax(trm1,1e-8)
    trm1 = qnorm(trm1)
    trm2 = pmin(t2.cdf(T2),1-1e-8)
    trm2 = pmax(trm2,1e-8)
    trm2 = qnorm(trm2)
    qscore = theta1*trm1 + theta2*trm2 + Z
    u.R = q.cdf(qscore)
  }
  else {
    u.R = runif(nnew)
  ...
  #R_setting=0 gives fixed recruitment age of L_R
  #R_setting=1 gives uniform distribution on (L_R,U_R)
  #R_setting=2 uses the marginal recruitment age distribution of UKB
  if (R_setting==0) R = L_R
  if (R_setting==1) R = L_R + (U_R-L_R)*u.R
  if (R_setting==2) {
    rct_ch = R_group$ch
  ...
  #COMPUTE ENTRY PROBABILITY
  if (pr.enter.flg) { 
    pr.enter <<- mean(data$T2>data$R)
    print(noquote(paste0('pr.enter = ', pr.enter)))
  }  
  
  #COMPUTE CORRELATIONS
  if (cortst) {
    cvmt = cov(cbind(T1,T2))
    T1mod = T1 - (cvmt[1,2]/cvmt[2,2])*T2
    T2mod = T2 - (cvmt[1,2]/cvmt[1,1])*T1
    print(cor(T1mod,R))
    print(cor(T2mod,R))
    print(noquote('Correlation computation completed'))
    stopQuietly()
  } 
 
  # Generate C = age at censoring
  if (C_setting == 0) data$C <- rep(Inf,nnew) 
  if (C_setting == 1) data$C <- data$R + runif(nnew,11,15)  #as in UKB   
  if (C_setting == 2) data$C <- data$R + runif(nnew,11,25)  #longer follow-up
...
samgen = function(nsam, pr.enter, T1_setting, T2_setting, R_setting,
  C_setting, L_R, U_R, t2.cdf, q.edf, round.flg) {

  #GENERATE THE SAMPLE
  ngen = round(1.30 * nsam / pr.enter)  
  sam.ini = sim_data(ngen, T1_setting, T2_setting, R_setting, 
    C_setting, L_R, U_R, t2.cdf, q.cdf, pr.enter.flg=FALSE)
...
if (popgen) psetup(nn,T1_setting,T2_setting)
if (!popgen) {
  ciftru.dat = read.csv(ciftru.file.name)
  ciftru = ciftru.dat[,2]
  ciftru.aj = ciftru.dat[,3]
  if (icorr) t1t2z.dat = read.csv(t1t2z.file.name)
}

#FOR RUNS WITH CORRELATED R, SET UP CDF OF THE SCORE USED TO GENERATE R
t2.cdf = NULL
q.cdf = NULL
if (icorr) {
  t2.cdf = ecdf(t1t2z.dat$T2)
  q.cdf = q.cdf.gen(t1t2z.dat,t2.cdf)
}

#COMPUTE ENTRY PROBABILITY
sdat = sim_data(nn=1e6,T1_setting,T2_setting,R_setting,C_setting,
  L_R,U_R,t2.cdf,q.cdf,pr.enter.flg=TRUE) 
...
for (irep in 1:nrep) {
  ...
  datcur = samgen(n, pr.enter, T1_setting, T2_setting, R_setting,
    C_setting, L_R, U_R, t2.cdf, q.edf, rnddat) 
...
#PLOT MEANS
if (!ipre & !cmb.flg) {
  temp = cbind(c(tru.lab,aj.lab,new.lab), c(tgrd,tgrd,tgrd), 
               c(ciftru[tgrd],cif.mean.aj.est,cif.mean.new.est))
  lev.vec = c('True', 'AJ', 'New')
...
if (!ipre & !cmb.flg) {
  temp = cbind(c(tru.lab,aj.lab,new.lab), c(tgrd,tgrd,tgrd), 
    c(ciftru[tgrd],cif.med.aj.est,cif.med.new.est))
  lev.vec = c('True', 'AJ', 'New')
  col.vec = c('black', 'blue', 'red')
  typ.vec = c('solid', 'dashed', 'dotted')
  lw.vec = c(lwrest, lwrest, lwdot)
...
#PLOT IQR'S
if (!ipre & cmb.flg) {
  cbind(c(aj.lab,new.lab,comb.lab), c(tgrd,tgrd,tgrd), 
...
if (!ipre & !cmb.flg) {
  temp = cbind(c(aj.lab,new.lab), c(tgrd,tgrd), c(cif.iqr.aj,cif.iqr.new))
  lev.vec = c('AJ', 'New')
  col.vec = c('blue', 'red')
  typ.vec = c('dashed', 'dotted')
  lw.vec = c(lwrest, lwdot)
...
if (!ipre) {
  ...
  if (!cmb.flg) {
    temp = cbind(c(aj.lab[ixcb1],new.lab[ixcb1]), c(tgrd1,tgrd1), 
      c(cvr.rte.ptwise.aj.3[ixcb1],cvr.rte.ptwise.new[ixcb1]))
    lev.vec = c('AJ', 'New', 'Comb')
    col.vec = c('blue', 'red')
    typ.vec = c('dashed', 'dotted')
    lw.vec = c(lwrest, lwdot)
...
if (!ipre) {
  ...
  if (!cmb.flg) {
    temp = cbind(c(aj.lab[ixcb1],new.lab[ixcb1]), c(tgrd1,tgrd1), 
                 c(cif.mean.width.aj[ixcb1],cif.mean.width.new[ixcb1]))
    lev.vec = c('AJ', 'New', 'Comb')
    col.vec = c('blue', 'red')
    typ.vec = c('dashed', 'dotted')
    lw.vec = c(lwrest, lwdot)
...
if (T1_setting != 3) {
  ...
  if (cmb.flg) txt3 = noquote(paste0('Combo: ', round(cvr.rte.band.cmb, digits=3)))
}
if (T1_setting == 3) {
...
if (!ipre) {
  ...
  if (!cmb.flg) {
    temp = cbind(c(aj.lab[ixcb2],new.lab[ixcb2]), c(tgrd2,tgrd2), 
      c(cif.mean.band.width.aj,cif.mean.band.width.new))
    lev.vec = c('AJ', 'New')
    col.vec = c('blue', 'red')
    typ.vec = c('dashed', 'dotted')
    lw.vec = c(lwrest, lwdot)
...
if (icorr) {
  ...
  pprplt = ggarrange(plt.means, nrow=1, ncol=1, legend='none')
